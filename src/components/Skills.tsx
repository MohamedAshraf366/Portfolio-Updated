...
const skills: Skill[] = [
  ...
  { name: "Bootstrap", icon: "https://img.icons8.com/?size=100&id=bYg7whCBKVmt&format=png", level: 88, color: "#7952B3" },
...
const SkillCard: React.FC<{ skill: Skill; index: number }> = React.memo(({ skill, index }) => {
  ...
  return (
    <motion.div
      ...
      className="group glass rounded-2xl p-5 hover:border-cyan-500/30 hover:bg-white/5 transition-all duration-300 hover:-translate-y-1 cursor-default relative overflow-hidden"
      style={{
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(400px circle at ${(tilt.y / 15 + 0.5) * 100}% ${(tilt.x / -15 + 0.5) * 100}%, ${skill.color}15, transparent 60%)`,
        }}
      />

      <div className="flex items-center gap-4 mb-4 relative z-10">
        <div
          className="w-12 h-12 rounded-xl flex items-center justify-center p-2 group-hover:scale-110 transition-transform duration-300"
      ...
      <div className="h-1.5 rounded-full bg-white/5 overflow-hidden relative z-10">
        <motion.div
          ...
          style={{ backgroundColor: skill.color, boxShadow: `0 0 12px ${skill.color}60` }}
        />
      </div>
    </motion.div>
  );
...
const MARQUEE_TECH = ["Express.js", "PostgreSQL", "Redis", "Docker", "REST APIs", "GraphQL", "Jest", "Webpack", "Vite", "AWS S3"];
  ...
  return (
    <section id="skills" className="py-28 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4" style={{ perspective: "1000px" }}>
          {skills.map((skill, i) => (
        ...
        <div className="mt-16 overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex gap-6 items-center animate-[marquee_20s_linear_infinite] w-max">
              {[...MARQUEE_TECH, ...MARQUEE_TECH].map((tech, i) => (
                <span
                  key={`${tech}-${i}`}
                  className="whitespace-nowrap px-4 py-2 rounded-full border border-white/10 text-slate-500 text-xs font-mono hover:text-slate-300 hover:border-white/20 hover:bg-white/5 transition-all duration-300 cursor-default"
                >
                  {tech}
                </span>
              ))}
